defmodule EventsApp.Invites do
  ...
  def list_invites_by_event(event_id) do
    Repo.all(from(EventsApp.Invites.Invite, where: [event_id: ^event_id]))
    |> Repo.preload(:user)
    |> Repo.preload(:event)
  end

  def list_invites_by_user(user_id) do
    Repo.all(from(EventsApp.Invites.Invite, where: [user_id: ^user_id]))
    |> Repo.preload(:user)
    |> Repo.preload(:event)
  end
